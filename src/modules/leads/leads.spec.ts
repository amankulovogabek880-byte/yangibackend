...
describe('Lead source validation', () => {
  it('INSTAGRAM valid', () => expect(safeEnum('INSTAGRAM', LEAD_SOURCES, 'OTHER')).toBe('INSTAGRAM'));
  it('WHATSAPP valid', () => expect(safeEnum('WHATSAPP', LEAD_SOURCES, 'OTHER')).toBe('WHATSAPP'));
  it('TELEGRAM valid', () => expect(safeEnum('TELEGRAM', LEAD_SOURCES, 'OTHER')).toBe('TELEGRAM'));
  it('UNKNOWN -> OTHER', () => expect(safeEnum('UNKNOWN', LEAD_SOURCES, 'OTHER')).toBe('OTHER'));
  it('undefined -> OTHER', () => expect(safeEnum(undefined, LEAD_SOURCES, 'OTHER')).toBe('OTHER'));
});

describe('Round-robin assignment', () => {
  function roundRobin(agents: string[], lastIndex: number): { agentId: string; nextIndex: number } {
    if (!agents.length) throw new Error('Agentlar yo\'q');
  ...
  it('navbat bilan o\'tadi', () => {
    ...
    expect(assigned[3]).toBe('agent-1'); // takrorlanadi
  });
