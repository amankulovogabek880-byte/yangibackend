import { calculateLeadScore, paginate, clean } from '../../common/utils/helpers';

// calculateLeadScore testlari
describe('calculateLeadScore', () => {
  ...
  it('REFERRAL source yuqori ball beradi', () => {
    const s1 = calculateLeadScore({ source: 'REFERRAL' });
    const s2 = calculateLeadScore({ source: 'OTHER' });
    expect(s1).toBeGreaterThan(s2);
  });

  it('VIP tier yuqori ball beradi', () => {
    const s1 = calculateLeadScore({ tier: 'VIP' });
    const s2 = calculateLeadScore({ tier: 'REGULAR' });
    expect(s1).toBeGreaterThan(s2);
  });

  it('email bo\'lsa ball oshadi', () => {
    const s1 = calculateLeadScore({ email: 'test@test.com' });
    const s2 = calculateLeadScore({ email: null });
    expect(s1).toBeGreaterThan(s2);
  });

  it('passport bo\'lsa ball oshadi', () => {
    const s1 = calculateLeadScore({ passportNo: 'AA1234567' });
    const s2 = calculateLeadScore({ passportNo: null });
    expect(s1).toBeGreaterThan(s2);
  });

  it('eski kontakt ball kamaytiradi', () => {
    const s1 = calculateLeadScore({ daysSinceContact: 0 });
    const s2 = calculateLeadScore({ daysSinceContact: 90 });
    expect(s1).toBeGreaterThan(s2);
  });

  it('bir nechta booking yuqori ball', () => {
    const s1 = calculateLeadScore({ totalBookings: 5, totalRevenue: 10000 });
    const s2 = calculateLeadScore({ totalBookings: 0, totalRevenue: 0 });
    expect(s1).toBeGreaterThan(s2);
  });

  it('score 0-100 orasida bo\'ladi', () => {
    const extremeHigh = calculateLeadScore({
    ...
    const extremeLow = calculateLeadScore({
...
describe('paginate edge cases', () => {
  ...
  it('string sahifa ishlaydi', () => {
    expect(paginate('3', '10').page).toBe(3);
    expect(paginate('3', '10').skip).toBe(20);
  });
  it('juda katta limit cheklanadi', () => {
    expect(paginate(1, 10000).limit).toBe(100);
  });
});
...
describe('clean object', () => {
  it('undefined olib tashlanadi', () => {
    const r = clean({ a: 1, b: undefined, c: 0, d: '', e: null, f: false });
