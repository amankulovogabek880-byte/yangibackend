// WhatsApp service logic testlari
...
describe('WhatsApp config validation', () => {
  function validateConfig(cfg: { instanceId?: string; token?: string }) {
    if (!cfg.instanceId?.trim()) return 'instanceId majburiy';
    if (!cfg.token?.trim()) return 'token majburiy';
    return null;
  }
